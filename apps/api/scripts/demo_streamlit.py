...
# Header Tampilan
st.title("🏛️ Senta Asisten Cerdas Sentuh Tanahku")
...
if prompt := st.chat_input("Ketik pertanyaan Anda di sini..."):
    ...
    # 1. Ekstrak riwayat chat SEBELUM pertanyaan baru ditambahkan
    # Kita ambil dari session_state dan format ke dalam list of dicts
    riwayat_chat = [
        {"role": msg["role"], "content": msg["content"]} 
        for msg in st.session_state.messages
    ]
    
    # 2. Tampilkan pertanyaan user di layar & simpan ke session state
    st.chat_message("user").markdown(prompt)
    st.session_state.messages.append({"role": "user", "content": prompt})

    # 3. Kirim ke API Render (Backend Anda)
    with st.chat_message("assistant"):
        with st.spinner("Senta sedang mengetik..."):
            try:
                # Payload kini memiliki parameter "riwayat"
                payload = {
                    "pesan": prompt,
                    "session_id": st.session_state.session_id,
                    "riwayat": riwayat_chat  # <--- INI KUNCI INGATANNYA
                }
                ...
                if response.status_code == 200:
                    data = response.json()
                    jawaban_ai = data.get("jawaban", "Maaf, Senta tidak bisa menemukan jawaban.")
                    ...
                    # Simpan jawaban AI ke riwayat session state
